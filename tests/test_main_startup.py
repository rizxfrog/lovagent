...
class MainStartupTests(unittest.IsolatedAsyncioTestCase):
    async def test_lifespan_logs_error_when_database_check_fails(self):
        ...
        with (
            ...
            patch("app.main.check_database_connection", return_value=(False, "connect failed")),
            ...
            patch(
                ...
                AsyncMock(return_value={"ok": True, "detail": ""}),
            ),
        ...
        error_mock.assert_any_call("Startup database availability check failed: %s", "connect failed")

    async def test_lifespan_logs_exception_when_database_check_crashes(self):
        scheduler_started = asyncio.Event()
        scheduler_cancelled = asyncio.Event()
        ...
        with (
            patch("app.main.init_db"),
            patch("app.main.check_database_connection", side_effect=RuntimeError("boom")),
            patch("app.main.tunnel_service.ensure_started"),
            patch(
                "app.main.tunnel_service.get_status",
                return_value={"public_url": ""},
        ...
        await asyncio.wait_for(scheduler_cancelled.wait(), timeout=1.0)
        exception_mock.assert_any_call("Startup database availability check crashed")
    ...
    async def test_lifespan_logs_error_when_model_check_fails(self):
        ...
        with (
            patch("app.main.init_db"),
            patch("app.main.check_database_connection", return_value=(True, None)),
            patch("app.main.tunnel_service.ensure_started"),
            patch(
                "app.main.tunnel_service.get_status",
                return_value={"public_url": ""},
